...
日期：2026-09-09  
状态：用户于 2026-09-09 要求暂停项目；阶段 2 尚未完成。

## 暂停时运行状态

- 已停止本次预览进程 `25328`，确认没有残留的 `CodexUsagePet` 进程。
- WPF 运行时已由矢量占位角色切换为当前真实风堇 8×11 图集。
- 本地预览映射：召唤使用 `waving`、待机使用 `idle/review`、悬停使用 `waiting`、点击使用 `jumping`、无聊和阈值提醒使用 `failed`。
- 预览版构建结果为 0 警告、0 错误，原有行为测试 9/9 通过。
- 预览素材保存于 `assets/pets/fengjin/spritesheet-preview.png`；它仍带有下述 5 像素正式验证阻塞，不应当作最终发布图集。
